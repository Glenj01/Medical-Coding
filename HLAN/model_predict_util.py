...
# def load_icd_data(path_icd_diag='../knowledge_bases/D_ICD_DIAGNOSES.csv', path_icd_proc='../knowledge_bases/D_ICD_PROCEDURES.csv'):
#     diagnosis_dict = {}
#     procedure_dict = {}

#     # Load diagnosis data
#     map_icd_diag = pd.read_csv(path_icd_diag)
#     for index, row in map_icd_diag.iterrows():
#         icd9_code = row['ICD9_CODE']#.replace('.', '')  # Remove the period from the ICD code
#         diagnosis_dict[icd9_code] = {
#             'short_title': row['SHORT_TITLE'].strip(),
#             'long_title': row['LONG_TITLE'].strip()
#         }

#     # Load procedure data
#     map_icd_proc = pd.read_csv(path_icd_proc)
#     for index, row in map_icd_proc.iterrows():
#         icd9_code = row['ICD9_CODE']#.replace('.', '')  # Remove the period from the ICD code
#         procedure_dict[icd9_code] = {
#             'short_title': row['SHORT_TITLE'].strip(),
#             'long_title': row['LONG_TITLE'].strip()
#         }

#     return {'diagnosis': diagnosis_dict, 'procedure': procedure_dict}

def retrieve_icd_descs(str_icd_code): #icd_data):
    if str_icd_code == '':
        return '', '', ''

    code_type = check_code_type(str_icd_code)
    str_icd_code = str_icd_code.replace('.', '')  # Remove the period from the ICD code
    
    #print(str_icd_code)
    # with open('../knowledge_bases/D_ICD_DIAGNOSES.csv', 'r') as file:
    #         reader = csv.DictReader(file)
    #         for row in reader:
    #             #print(row)
    #             if row['ICD9_CODE'] == str_icd_code:
    #                 #print(f"code {'ICD9_CODE'}, title{'LONG_TITLE'}")
    #                 return row['SHORT_TITLE'], row['LONG_TITLE'], code_type
    #         print("reached the end of diag file") 
    if code_type == 'diag':
        #def get_desc(searchValue, filepath):
        #str_icd_code = '01166'
        with open('../knowledge_bases/D_ICD_DIAGNOSES.csv', 'r') as file:
            reader = csv.DictReader(file)
            for row in reader:
        ...
        # if str_icd_code in icd_data['diagnosis']:
        #     return icd_data['diagnosis'][str_icd_code]['short_title'], icd_data['diagnosis'][str_icd_code]['long_title'], code_type
        # else:
        #     return None, None, None
    elif code_type == 'proc':
        with open('../knowledge_bases/D_ICD_PROCEDURES.csv', 'r') as file:
            reader = csv.DictReader(file)
            for row in reader:
                #print(row)
                if row['ICD9_CODE'] == str_icd_code:
                    #print(f"code {'ICD9_CODE'}, title{'LONG_TITLE'}")
                    return row['SHORT_TITLE'], row['LONG_TITLE'], code_type
            #print("reached the end of procedures file")    
            
        # if str_icd_code in icd_data['procedure']:
        #     return icd_data['procedure'][str_icd_code]['short_title'], icd_data['procedure'][str_icd_code]['long_title'], code_type
        # else:
        #     return None, None, None
    else:
        return None, None, None
